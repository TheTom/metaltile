...
pub fn ffai_batched_4_qgemv_fast<T>(
    ...
    if base_row < out_limit {
        ...
        for _r in range(0u32, 4u32, 1u32) {
            ...
            let r = simd_sum(v);
            if lane == 0u32 {
                if matrix == 0u32 { store(a_out[base_row + _r], r.cast::<T>()); }
                if matrix == 1u32 { store(b_out[base_row + _r], r.cast::<T>()); }
                if matrix == 2u32 { store(c_out[base_row + _r], r.cast::<T>()); }
                if matrix == 3u32 { store(d_out[base_row + _r], r.cast::<T>()); }
            }
        }
    }
}
