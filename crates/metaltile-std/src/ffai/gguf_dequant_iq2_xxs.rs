...
//!
//! Follows the canonical `dequantize_row_iq2_xxs` reference algorithm.
//!
...
//! stubbed pending the canonical `iq2xxs_grid` table (256 × 8-byte
//! signed-octet entries) — see TODO blocks below.
//!
...
//
// The dequant uses **two** runtime lookup tables from the
// canonical IQ2_XXS reference algorithm (Apache-2.0 here):
//
...
pub mod kernel_tests {
    //! **WIP**: end-to-end correctness ignored pending the canonical
    //! `iq2xxs_grid` table. The kernel itself compiles + dispatches
    //! cleanly; the codegen-shape invariants below catch ABI / IR
    //! regressions even without the grid table.
    ...
    /// TODO end-to-end correctness vs the canonical reference. The
    /// grid + ksigns tables ship on the FFAI loader side. Once they're
    /// wired through Ops.swift this fixture should round-trip a
    /// quantized + dequantized block
