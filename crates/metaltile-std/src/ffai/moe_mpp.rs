...
pub fn kernel_ir_for(dt: DType) -> Kernel {
    assert!(
        ...
        "mt_moe_gather_qmm_mma_int4_bm16_mpp: dtype must be F32 or F16, got {:?}",
        dt
    );
    let t = match dt {
        DType::F32 => "float",
    ...
    //  2. Force `tgid_y` alias emission by referencing axis=1 ProgramId.
    //     (`tgid_x` alias is unconditional in Reduction mode.) We push
    //     both axes as no-op ops with thrown-away results — the
    //     `Op::ProgramId` emitter writes `uint v = tgid_x;` /
    //     `uint v = tgid_y;`, which is fine: unused locals don't cost
    //     anything past `-Wno-unused-variable` (Metal accepts them).
    ...
    // Tickle the codegen to emit `tgid_y` aliases. Without this, the
    // `kernel_uses_program_id_axis(kernel, 1)` check in
    // `codegen/msl/mod.rs` returns false and `tgid_y` is omitted from
    // the preamble — the inline MSL would then fail to compile because
    // it references `tgid_y` directly. The synthetic ProgramId result
    // value is unreferenced by anything downstream; the generated
    // `uint v = tgid_y;` line is dead but harmless (Metal accepts
    // unused locals). `tgid_x` is unconditional in Reduction mode so
    // no hint needed.
