...
pub mod kernel_tests {
    ...
    // tol 2e-4: sqrtsoftplus routes through exp + sqrt, so the GPU↔CPU-oracle
    // gap is dominated by transcendental fast-math and varies by GPU family
    // (~5e-7 on M5, ~6e-5 on older Apple GPUs). 2e-4 holds across hardware
    // while staying far tighter than a logic error would ever land.
