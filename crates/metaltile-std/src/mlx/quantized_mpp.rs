...
fn substitute_dtype(src: &str, dt: DType) -> String {
    let t = match dt {
        DType::F32 => "float",
        DType::F16 => "half",
        _ => panic!("mt_qmm_mma_mpp only supports F32 / F16, got {:?}", dt),
    };
    src.replace("{T}", t)
}
...
pub fn kernel_ir_for(dt: DType) -> Kernel {
    ...
    // Single InlineMsl op. We force the kernel to emit a `tgid_y` reference
    // by also pushing an `Op::Load { src: "tgid_y", indices: [] }` ... but
    // there's a simpler path: `kernel_uses_program_id_axis` only checks
    // for `Op::ProgramId { axis: 1 }` OR `Op::Load { src: "tgid_y" }`. The
    // InlineMsl source mentioning `tgid_y` doesn't trip that check, so we
    // must enable the emit explicitly. We do this by inserting a synthetic
    // `Op::ProgramId { axis: 1 }` before the InlineMsl — its result value
    // is unused but it forces `tgid_y` to be defined in the kernel body.
    // (Reduction mode emits `tgid_x` unconditionally, so axis=0 needs no
    // such hint.) The vname allocator gives it a unique SSA id so it
    // doesn't collide with InlineMsl identifiers.
...
mod tests {
    ...
    fn kernel_ir_constructs_for_f32_and_f16() {
        for dt in [DType::F32, DType::F16] {
            ...
            // Body has ProgramId(1) + InlineMsl.
            assert!(k.body.ops.iter().any(|op| matches!(op, Op::InlineMsl { .. })));
            assert!(k.body.ops.iter().any(|op| matches!(op, Op::ProgramId { axis: 1 })));
        }
    }
