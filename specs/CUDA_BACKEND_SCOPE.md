...
- [x] **Phase 2 (started) — reduction family GREEN on GX10.** `row_reduce_sum` (KernelMode::Reduction): `StrideReduce` (per-thread grid-stride accumulation) + `Reduce` (warp `__shfl_down_sync` + `__shared__` tree across warps + `__syncthreads`) — the CUDA analog of `msl::reduce`'s two-level `simd_sum`. **max|Δ| = 4.1e-8** over 128 rows. The 32-lane warp ≙ 32-lane simdgroup match made it port cleanly. Mode-aware signature (no `_n_elems` for Reduction) + reduction preamble (tid/tgid/lsize/n_simd/simd_lane/simd_group). 173 codegen + 3 hardware tests green.
